    ...
    if missing_columns:
        ...
        if selected_theme:
            ...
            for _, row in theme_data.iterrows():
                ...
                responses.append({
                    "response": f"*In {building_name}, {response_text}*",
        ...
        if selected_building:
            ...
            if avg_sentiment > 0.2:
                recommendation = (
                    f"The sentiment for {selected_building} is largely positive. Maintain strengths in design "
                    "and enhance by adding collaborative or innovative features."
                )
            elif -0.2 <= avg_sentiment <= 0.2:
                recommendation = (
                    f"The sentiment for {selected_building} is neutral. Consider improvements in lighting, "
                    "seating, and overall accessibility to enhance user satisfaction."
                )
            else:
                recommendation = (
                    f"The sentiment for {selected_building} is negative. Focus on redesigning outdated spaces, "
                    "improving furniture quality, and adding collaborative tools."
                )
