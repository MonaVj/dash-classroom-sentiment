    ...
    try:
        ...
        if missing_columns:
            ...
            if theme_data.empty:
                ...
                responses = [{"response": f"*In {row['Buildings Name']}, {row['Tell us about your classroom']}*", "sentiment": "🟢" if row["Avg_Sentiment"] > 0.2 else "🟠" if -0.2 <= row["Avg_Sentiment"] <= 0.2 else "🔴"} for _, row in theme_data.iterrows()]
